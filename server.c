...
void shared_memory_write(SimulationState *state, sharedData *data, int walker_x, int walker_y, int current_rep) {
    ...
    if (state->interactive_mode) {
        ...
        for (int x = 0; x < state->world_width_; x++) {
            for (int y = 0; y < state->world_height_; y++) {
                snprintf(data->grid[x][y], 5, "%.2f", state->prob_world[x][y]);
            }
        }
    }
...
int main(void) {
    ...
    //inicializacia shm
    shared_memory_write(&state, shm_data, 0, 0, 0);
